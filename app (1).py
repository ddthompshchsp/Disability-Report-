...
# Header bar (logo left, centered title, timestamp)
col_logo, col_title, col_spacer = st.columns([1,4,1], vertical_alignment="center")
with col_logo:
    logo = st.file_uploader("Logo (PNG, optional)", type=["png"], key="logo", label_visibility="collapsed")
with col_title:
    st.markdown("<h1 style='text-align:center;margin-bottom:0'>HCHSP – Disability Report</h1>", unsafe_allow_html=True)
    st.markdown(f"<p style='text-align:center;color:#555;margin-top:2px'>Exported on: {datetime.now().strftime('%m/%d/%y %I:%M %p')} Central</p>", unsafe_allow_html=True)
...
with st.sidebar:
    st.header("Inputs")
    uploaded = st.file_uploader("GEHS Quick Report (xlsx) – headers on row 5", type=["xlsx"], key="raw")
    st.caption("Tip: Run the same extract you used for the final Excel.")
...
def process(file_bytes: bytes):
    ...
    # Inclusion: require IEP/IFSP:Form Date (Column I logic)
    ...
    # Merge dups left->right with commas; format dates mm/dd/yy
    ...
    # Auth formatting for UI table (Excel export will still show red via conditional formatting)
    ...
    # Order columns: PID, Participant, Center, Class, then rest
    ...
    # Drop columns R, S, and M by Excel positions
    ...
    # Disability breakdown: unique + Unspecified
    ...
    return clean[final_cols], centers, disab_breakdown, (auth_col if auth_col in final_cols else None)

def build_excel(summary_df: pd.DataFrame, centers_df: pd.DataFrame, disab_df: pd.DataFrame, auth_col_name: str | None, logo_bytes: bytes | None):
    ...
    with pd.ExcelWriter(out_buf, engine="xlsxwriter") as writer:
        ...
        ts = datetime.now().strftime("%m/%d/%y %I:%M %p Central")
        ...
        ws1.merge_range(2, 1, 2, max(1, summary_df.shape[1]-1), f"Exported on: {ts}", sub_fmt)
        if logo_bytes:
        ...
        for j, h in enumerate(summary_df.columns):
            ws1.write(5, j, h, header_fmt); ws1.set_column(j, j, 22)
        ...
        if auth_col_name is not None:
            ...
            ws1.conditional_format(6, auth_idx, 6+len(summary_df), auth_idx, {"type":"cell","criteria":"equal to","value":'"X"',"format": redx_fmt})
        ...
        ws2.merge_range(2, 1, 2, 10, f"Exported on: {ts}", sub_fmt)
        if logo_bytes:
        ...
        for j, h in enumerate(centers_df.columns):
            ws2.write(5, j, h, header_fmt); ws2.set_column(j, j, 32 if j==0 else 24, pct_fmt if j>=2 else None)
        ...
        ws3.merge_range(2, 1, 2, 12, f"Exported on: {ts}", sub_fmt)
        if logo_bytes:
        ...
        chart1.add_series({
            "name": "Current Identified",
            "categories": ["Dashboard", 6, 0, 6, 0],
            "values":     ["Dashboard", 6, 1, 6, 1],
            "data_labels": {"value": True, "font": {"bold": True}}
        })
        chart1.add_series({
            "name": "Target (10%)",
            "categories": ["Dashboard", 5, 0, 5, 0],
            "values":     ["Dashboard", 5, 1, 5, 1],
            "data_labels": {"value": True, "font": {"bold": True}}
        })
...
# Process now that we have a file
if uploaded is None:
    st.info("Upload the raw GEHS Quick Report (xlsx) in the sidebar to begin.")
    st.stop()
...
with tab_dash:
    ...
    with c1:
        st.metric("Current Identified", len(df_view))
    with c2:
        st.metric("Target (10%)", TARGET)
    with c3:
        pct = (len(df_view)/ENROLLMENT) if ENROLLMENT else 0
        st.metric("% of Enrollment", f"{pct:.2%}")
    st.caption("Charts export with labels/formatting identical to your Excel reports.")
...
with tab_export:
    exp_logo = None
    if st.session_state.get("logo") is not None:
        exp_logo = st.session_state["logo"].getvalue()
    xlsx_buf = build_excel(df_summary, df_centers, df_disab, auth_col_name, exp_logo)
    st.download_button("Download Excel Export", data=xlsx_buf, file_name=f"HCHSP_Disability_Export_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
