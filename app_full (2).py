...
st.markdown(
    """
    <style>
    .hero {text-align:center; margin-top:24px; margin-bottom:8px;}
    .hero img {width: 220px;}
    .hero h1 {font-size: 42px; line-height: 1.15; margin: 18px 0 0 0;}
...
def process(file_bytes: bytes):
    ...
    # Inclusion: must have an IEP/IFSP Form Date (Column I logic)
    ...
    # Merge duplicates: left->right, join with commas; format date-like columns as mm/dd/yy
    ...
    # Authorization formatting (UI shows the value; Excel adds red X via conditional formatting)
    ...
    # Column order: PID, Participant, Center, Class, then rest (original order)
    ...
    # Drop columns R & S by Excel position and also drop column M
    ...
    # Disability breakdown (unique per student, "Unspecified" for blanks)
...
def build_excel(summary_df: pd.DataFrame, centers_df: pd.DataFrame, disab_df: pd.DataFrame, auth_col_name: str | None, logo_bytes: bytes | None):
    ...
    with pd.ExcelWriter(out_buf, engine="xlsxwriter") as writer:
        ...
        # Sheet2: Center Totals (with filter & dynamic totals)
        ...
        # Chart 3: Disability Type Breakdown (Unique per Student)
